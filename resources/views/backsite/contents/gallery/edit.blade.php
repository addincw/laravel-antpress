...
@section('content')
<form class="form" action="{{ url($route . '/' . $gallery->id) }}" method="post" enctype="multipart/form-data">
  {{ csrf_field() }}
  {{ method_field('PUT') }}

  @component('backsite.contents.gallery._form', ['contents' => $contents, 'gallery' => $gallery]) @endcomponent
</form>
@endsection
