...
<div class="card">
  <div class="card-header">
    <h4 class="card-title">
      Debitur
      <a class="btn btn-primary white pull-right" href="{{ url($route . '/create') }}">Tambah Debitur <i class="ft-plus"></i> </a>
    </h4>
  </div>
  <div class="card-body">
      ...
      <table id="recent-orders" class="table table-striped table-bordered table-hover table-xl mb-0">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>
              {!! html_entity_decode(str_limit(strip_tags($debitur->description), 150)) !!}
              @if (strlen(strip_tags($debitur->description)) > 150)
              ...
              @endif
            </td>
            <td class="text-truncate text-center">
              <a href='{{ url("$route/$debitur->id/edit") }}' class="btn btn-sm btn-icon btn-pure info"><i class="ft-edit"></i></a>
              <a href="{{ url($route . '/' . $debitur->id) }}" class="btn btn-sm btn-icon btn-pure danger deleteBtn"><i class="ft-trash"></i></a>
            </td>
          </tr>
          @endforeach
        </tbody>
